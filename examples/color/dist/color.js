(function () {
    ...
    ColorModel.inject = ['emitter'];
    class ContainerMediator {
        constructor(target, mediators) {
            for (let i = 0; i < 20; i++) {
                $(target).append('<div class="widget" data-id="' + i + '">' +
    ...
    ContainerMediator.inject = ['target', 'mediators'];
    class WidgetMediator {
        constructor(target, emitter, colorModel) {
            const id = $(target).attr('data-id');
            emitter.addListener('all', () => {
                $(target).css('background-color', colorModel.getColor());
            });
            emitter.addListener('others', (data) => {
                if (id !== data?.id) {
                    $(target).css('background-color', colorModel.getColor());
                }
            });
    ...
    WidgetMediator.inject = ['target', 'emitter', 'colorModel'];
