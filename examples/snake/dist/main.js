(function () {
    ...
    class StartCommand {
        constructor(_input, _collision, _gridLayer, _snakeLayer, foodLayer, time, path) {
    ...
    StartCommand.inject = ['input', 'collision', 'gridLayer', 'snakeLayer', 'foodLayer', 'time', 'path'];
    ...
    class Path {
        constructor(grid, emitter, time) {
            ...
            this.emitter = emitter;
            this.reset();
            emitter.addListener('keydown', (data) => {
    ...
    Input.inject = ['emitter'];

    class Collision {
        constructor(snakeLayer, foodLayer, time, emitter) {
            time.addSpeedHandler(() => {
    ...
    Collision.inject = ['snakeLayer', 'foodLayer', 'time', 'emitter'];
    ...
    GridLayer.inject = ['context', 'time', 'grid', 'debug'];
    ...
    SnakeLayer.inject = ['context', 'grid', 'time', 'snake', 'path'];

    class FoodLayer {
        constructor(context, grid, time, food) {
            this.cell = { col: 0, row: 0 };
            this.context = context;
            this.grid = grid;
            this.food = food;
    ...
    FoodLayer.inject = ['context', 'grid', 'time', 'food'];
    ...
    class Canvas {
        constructor(target, injector, time, grid) {
    ...
    Canvas.inject = ['target', 'injector', 'time', 'grid'];
