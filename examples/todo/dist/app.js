(function () {
    ...
    const events$1 = settings.events = {};
    ...
    for (let i = 0, l = eventsArray.length; i < l; i++) {
        events$1[settings.eventsPrefix + eventsArray[i]] = eventsArray[i];
    }
    const regex = {
        sequence: null,
        token: null,
    ...
    function getNodeFromElement(element, scope) {
        ...
        for (let j = 0, jj = attrs && attrs.length; j < jj; j++) {
            ...
            if (events$1[attr.name] && newAttr) {
                if (events$1[attr.name] && !node.isRepeaterChild) {
                    eventsArray.push({ name: events$1[attr.name], value: attr.value, attr: newAttr });
                }
            }
        }
    ...
    function compileClone(node, newNode) {
        ...
        if (node.attributes) {
            for (let i = 0, l = node.attributes.length; i < l; i++) {
                ...
                if (events$1[attr.name] && newAttr) {
                    newNode.addEvent(events$1[attr.name], attr.value, newAttr);
                }
            }
        }
        // children
        ...
        for (let j = 0, jj = attrs && attrs.length; j < jj; j++) {
            ...
            if (events$1[name]) {
                ...
                if (handler && isFunction(handler)) {
                    addEvent(element, events$1[name], handler);
                    eventStore.push({ element: element, type: events$1[name], handler: handler });
                }
            }
        }
    }
    ...
    TodoAddCommand.inject = ['model'];
    class TodoRemoveCommand {
        constructor(model) {
            this.model = model;
        }
        execute(data) {
    ...
    TodoRemoveCommand.inject = ['model'];
    ...
    TodoUpdateCommand.inject = ['model'];
    class TodoToggleCommand {
        constructor(model) {
            this.model = model;
        }
        execute(data) {
    ...
    TodoToggleCommand.inject = ['model'];
    class TodoToggleAllCommand {
        constructor(model) {
            this.model = model;
        }
        execute(data) {
    ...
    TodoToggleAllCommand.inject = ['model'];
    class TodoClearCompletedCommand {
        constructor(model) {
            this.model = model;
        }
        execute() {
    ...
    TodoClearCompletedCommand.inject = ['model'];
    ...
    class Model {
        ...
        uuid() {
            let a = '';
            let b = '';
            for (a = b = '', a = 0; a++ < 36; b += a * 51 & 52 ? (a ^ 15 ? 8 ^ Math.random() * (a ^ 20 ? 16 : 4) : 4).toString(16) : '-')
                ;
            return b;
        }
    }
    Model.inject = ['emitter'];

    const ENTER_KEY = 13;
    function getLi(element) {
        if (!element)
            return null;
        return element.tagName === 'LI' ? element : getLi(element.parentElement);
    }
    function getActiveItems(data) {
        return data.filter((todo) => !todo.completed).length;
    }
    class TodoTemplate {
        constructor(scope, template, emitter) {
            emitter.addListener(events.RENDER, (data) => {
            ...
            scope.edit = (event, _todo) => {
                const li = getLi(event.currentTarget);
                if (li) {
                    li.classList.add('editing');
                }
            };
            scope.update = (event, id) => {
                const target = event.currentTarget;
                const value = target.value.trim();
            ...
            scope.remove = (_event, id) => {
                emitter.dispatch(events.REMOVE, { item: id });
            };
            scope.toggle = (_event, id) => {
    ...
    TodoTemplate.inject = ['scope', 'template', 'emitter'];
    ...
    class TodoApp extends Application {
        ...
        start() {
            const model = this.injector.getValue('model');
